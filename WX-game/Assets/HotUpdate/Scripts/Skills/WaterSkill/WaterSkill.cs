...
public class WaterSkill : BaseSkill
{
    ...
    {
        WaterSkillEffect Effect = PlayerController.Player.AddComponent<WaterSkillEffect>();
        SkillsData itemData = DataManager.Instance.GetData<SkillsData>("skill_tbskill", Data.id);
        if (itemData == null) { Debug.LogError($"[WaterSkill] 找不到技能数据，SkillData.id={Data.id}，请在 Inspector 中设置正确的 id"); return; }
